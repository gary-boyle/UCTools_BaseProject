    ...
    public class SaveFileDetailsPanel : VisualElement
    {
        ...
        private void CreateEnhancedDefaultFieldDisplays()
        {
            ...
            if (_currentSaveData != null)
            {
                ...
                if (_currentSaveData.RuntimeObjects != null && _currentSaveData.RuntimeObjects.Count > 0)
                {
        ...
        private void CreateRuntimeObjectsSection()
        {
            ...
            var runtimeObjects = _currentSaveData.RuntimeObjects;
        ...
        private void CreateRuntimeObjectDisplay(SerializedRuntimeObject obj, int index)
        {
            try
            {
                // Object header with type and ID
        ...
        private void CreateDeserializedObjectFields(SerializedRuntimeObject obj, int index)
        {
            try
            {
                ...
                if (deserializedObj != null)
                {
                    ...
                    {
                        CreateRuntimeObjectField($"Object[{index}].position", "Position", 
                            $"({deserializedObj.position.x:F2}, {deserializedObj.position.y:F2}, {deserializedObj.position.z:F2})");
                        CreateRuntimeObjectField($"Object[{index}].rotation", "Rotation", 
                            $"({deserializedObj.rotation.x:F1}°, {deserializedObj.rotation.y:F1}°, {deserializedObj.rotation.z:F1}°)");
                        CreateRuntimeObjectField($"Object[{index}].scale", "Scale", 
                            $"({deserializedObj.scale.x:F2}, {deserializedObj.scale.y:F2}, {deserializedObj.scale.z:F2})");
                        CreateRuntimeObjectField($"Object[{index}].isActive", "Is Active", deserializedObj.isActive);
                    }
                    
                    CreateRuntimeObjectField($"Object[{index}].prefabGUID", "Prefab GUID", 
                        string.IsNullOrEmpty(deserializedObj.prefabGUID) ? "<not set>" : deserializedObj.prefabGUID);
            ...
            try
            {
                ...
                if (specificFields.Length > 0)
                {
                    ...
                    foreach (var field in specificFields)
                    {
                        ...
                        var displayValue = FormatFieldValue(value, field.FieldType);
                        CreateRuntimeObjectField($"Object[{index}].{field.Name}", GetFriendlyFieldName(field.Name), displayValue);
                    }
                }
            }
            catch (Exception ex)
